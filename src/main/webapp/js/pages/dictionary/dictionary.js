		...
		var doSave = function(btn){
		 ...
		 if(btn.parent_id){
   		 ...
   		 $("#"+formId).validate("validate").done(function(result){
   			 if(result){
   				 $("#"+formId).ajaxSubmitForm(contextPath+"/dictionary/add",data,
	        				 function(){
	 			        		 $("#"+formId).dialog("close");
		...
		var addDicParam = function(){
			...
			$("#dictionaryParamForm").dialog({
				...
				afterShown:function(){
					$("#dictionaryParamForm").validate({align:'right',theme:"darkblue"});
				},
				beforeClose:function(){
		...
		var doUpdate = function(btn){
			 ...
			 data["dictionary.id"] = btn.data.id;
	   		 $("#dictionaryForm").validate("validate").done(function(result){
	   			 if(result){
	   				 $("#dictionaryForm").ajaxSubmitForm(contextPath+"/dictionary/update",data,
		        				 function(){
